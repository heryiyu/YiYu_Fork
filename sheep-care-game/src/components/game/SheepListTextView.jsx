    ...
    return (
        <div className={containerClass}>
            <div className={`sheep-lite-header ${isSelectionMode ? 'is-selection-mode' : ''}`}>
                ...
                <div className="lite-col-pin" title="釘選狀態"></div>
                <div className="lite-col-name">名字</div>
                <div className="lite-col-tag">標籤</div>
            ...
            <div className="sheep-lite-body">
                {sheepList.map(sheep => {
                    ...
                    const isPinned = pinnedSheepIds.includes(sheep.id);
                    ...
                    const tagLabel = currentIsSleeping ? '已沉睡' : (currentIsSick ? '生病' : '健康');

                    // For the dynamic style of tags
                    const statusBg = currentIsSleeping ? 'var(--tag-dead-bg)' : (currentIsSick ? 'var(--tag-sick-bg)' : 'var(--tag-healthy-bg)');
                    const statusText = currentIsSleeping ? 'var(--tag-dead-text)' : (currentIsSick ? 'var(--tag-sick-text)' : 'var(--tag-healthy-text)');
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="lite-col-pin">
                                <span
                                    className={`pinned-indicator ${isPinned ? 'active' : ''}`}
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        if (onTogglePin) onTogglePin(sheep.id);
                                    }}
                            ...
                            <div className="lite-col-tag">
                                {firstTag ? (
                                    <span className="lite-status-tag" style={{ background: firstTag.color || 'var(--palette-gray-muted)', color: 'var(--text-inverse)' }}>
                                        {firstTag.name}
                                    </span>
                                ) : (
                            ...
                            <div className="lite-col-health">
                                <span className="lite-status-tag" style={{ background: statusBg, color: statusText }}>
                                    {tagLabel}
                                </span>
                            </div>
                            {isLiteMode ? (
